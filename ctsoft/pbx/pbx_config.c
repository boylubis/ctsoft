...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 233240 $")
...
static char config[] = "ctsoft.conf";
...
static char *handle_cli_dialplan_save(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"\n"
			"Example: dialplan save                 (/etc/ctsoft/ctsoft.conf)\n"
			"         dialplan save /home/markster  (/home/markster/ctsoft.conf)\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	...
	if (a->argc == 3) {	/* have config path. Look for *.conf */
		...
		if (!strstr(a->argv[2], ".conf")) { /*no, this is assumed to be a pathname */
			...
			file = config;	/* default: 'ctsoft.conf' */
	...
	cfg = ast_config_load("ctsoft.conf", config_flags);
...
static char *handle_cli_dialplan_reload(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Reload ctsoft.conf without reloading any other\n"
			"       modules.  This command does not delete global variables\n"
			"       unless clearglobalvars is set to yes in ctsoft.conf\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
