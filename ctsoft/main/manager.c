...
static char *handle_showmancmd(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage = 
			...
			"	Shows the detailed description for a specific manager interface command.\n";
		return NULL;
	case CLI_GENERATE:
		l = strlen(a->word);
		which = 0;
...
#define HSMC_FORMAT "  %-15.15s  %-15.15s  %-55.55s\n"
	...
	case CLI_INIT:
		...
		e->usage = 
			...
			"	Prints a listing of all the available manager interface commands.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;	
	}	
...
static char *handle_showmaneventq(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage = 
			...
			"	Prints a listing of all events pending in the manger\n"
			"event queue.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
...
"Description: Checks if module is loaded\n"
"Variables: \n"
"  ActionID: <id>          Action ID for this transaction. Will be returned.\n"
"  Module: <name>          module name (not including extension)\n"
"\n"
...
"Description: Loads, unloads or reloads an module in a running system.\n"
"Variables: \n"
"  ActionID: <id>          Action ID for this transaction. Will be returned.\n"
"  Module: <name>          module name (including .so extension)\n"
...
static void *session_do(void *data)
{
	...
	astman_append(&s, "Call Manager/%s\r\n", AMI_VERSION);	/* welcome prompt */
	for (;;) {
...
#define TEST_STRING \
	...
	<input type=\"submit\">\n</form>\n"
		...
		ast_str_append(&out, 0, "<title>trade; Manager Interface</title>");
...
static int __init_manager(int reload)
{
	...
	if (!registered) {
		...
		ast_manager_register2("Command", EVENT_FLAG_COMMAND, action_command, "Execute CLI Command", mandescr_command );
	...
	if (!cfg || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log(LOG_NOTICE, "Unable to open configuration manager.conf, or configuration is invalid/disabled.\n");
		return 0;
	}
