...
int ast_agi_send(int fd, struct ast_channel *chan, char *fmt, ...)
{
	...
	if (agidebug) {
		if (chan) {
			ast_verbose("<%s>Tx >> %s", chan->name, ast_str_buffer(buf));
		} else {
			ast_verbose("Tx >> %s", ast_str_buffer(buf));
		}
	}
...
static int handle_exec(struct ast_channel *chan, AGI *agi, int argc, char **argv)
{
	...
	ast_verb(3, "Script Application: (%s) Options: (%s)\n", argv[1], argv[2]);
	...
	for (;;) {
		...
		if (c) {
			...
			while (buflen < (len - 1)) {
				...
				len -= buflen;
				if (agidebug)
					ast_verbose( "Rx << temp buffer %s - errno %s\n", buf, strerror(errno));
			}
			...
			if (!buf[0]) {
				...
				ast_verb(3, "<%s>Script %s completed, returning %d\n", chan->name, request, returnstatus);
				if (pid > 0)
